...
def test_download_postprocess_and_blank_detection(monkeypatch):
    ...
    # _post_process_image preserves native resolution (no forced resize)
    assert processed.size == downloaded.size
    assert processed.getpixel((0, 0))[3] == 255
    assert processed.getpixel((processed.size[0] - 1, processed.size[1] - 1))[3] == 255
